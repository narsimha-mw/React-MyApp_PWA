    ...
    <div className="dashboard-wrapper">
      ...
      <aside className="sidebar">
        <div className="sidebar-logo">
          ...
          <span className="sidebar-brand">MyApp</span>
        </div>
      ...
      <main className="dashboard-main">
        <header className="dash-header">
          <div>
            ...
            <p className="dash-subtitle">Welcome, <strong>{userEmail}</strong></p>
          </div>
        </header>
  ...
  return (
    <div className="profile-card">
      ...
      <div className="profile-info">
        ...
        <div className="info-row"><span>Phone</span><span>+1 (555) 000-1234</span></div>
        <div className="info-row"><span>Location</span><span>New York, USA</span></div>
      </div>
    </div>
  );
}

function ProductsTab({ products, onAdd, onEdit, onView, onDelete }) {
  return (
    <div>
          ...
          <tbody>
            {products.map((p, i) => (
              <tr key={p.id}>
                ...
                <td>{p.quantity}</td>
                <td>
                  <span className={`stock-badge ${p.stock < 15 ? 'low' : 'ok'}`}>{p.stock}</span>
                </td>
                <td>${p.price.toFixed(2)}</td>
                <td className="td-desc">{p.description}</td>
                <td>
                  <div className="action-btns">
                    <button className="btn-view" title="View" onClick={() => onView(p)}>View</button>
                    <button className="btn-edit" title="Edit" onClick={() => onEdit(p)}>Edit</button>
                    <button className="btn-danger-sm" title="Delete" onClick={() => onDelete(p.id)}>Delete</button>
                  </div>
                </td>
              </tr>
            ))}
...
function InvoicesTab({ invoices }) {
  return (
    <div className="table-wrapper">
      <table className="data-table">
        <thead>
          <tr><th>Invoice ID</th><th>Date</th><th>Amount</th><th>Status</th></tr>
        </thead>
        <tbody>
          {invoices.map(inv => (
            <tr key={inv.id}>
